...
This matches the boot code in `web/server/lore/memory/boot.ts`: runtime boot loads the global boot nodes `core://agent`, `core://soul`, and `preferences://user`, plus the active client-specific node such as `core://agent/codex` or `core://agent/claudecode`.
